...
mock_genes_directory="/hpc/shared/dbg_mz/marten/Crossomics_2019_12_10/Results/missed_seeds/"
...
mock_file_end=".txt"
for patient_number in {1..97}
do
    patient_seed_file=$mock_genes_directory$patient_file_begin$patient_number$patient_file_end
    if [ ! -f "$patient_seed_file" ]; then
        echo "all seeds available for patient $patient_number"
        continue
    fi
