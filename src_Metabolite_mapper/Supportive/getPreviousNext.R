getPreviousNext <- function(model, met_index, produced, max_rxns){
  ...
  for(j in 1:length(met_index)){
    ...
    message(as.vector(unlist(model$mets[met_index[j]], use.names = FALSE)))
    #     message(met_index)
    #     message(produced)
    ...
    if (max_rxns>=length(rxn_index)&length(rxn_index)>0){  
      for(k in 1:length(rxn_index)){
        ...
        if (produced) {
          mets = rbind(mets, getMetsRecon2V2(rxn_index[k], level=1, met, model)$metabolites)
