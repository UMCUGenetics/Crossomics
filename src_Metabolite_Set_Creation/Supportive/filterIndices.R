filter_indices <- function(model, shortmodel, met_index, max_rxns, min_rxns = 0){
  ...
  metSums <- rowSums(shortmodel[metNames,,drop = FALSE] != 0)
  # metNames_u_thres <- metNames[metSums > min_rxns & metSums <= max_rxns]
  metNames_u_thres <- metNames[metSums <= max_rxns]
