getSinglePatientGeneList <- function(patient){
  ...
  for (i in 1:length(patients)){
    ...
    gene_list = read.table(paste("./input/Patients_IV_HGNC.txt",sep=""), header = FALSE, sep="\t")
    # gene_list = read.table(paste("./db/P",patient,"_HGNC.txt",sep=""), header = FALSE, sep="\t")
    ensembl = useEnsembl(biomart="ensembl", dataset="hsapiens_gene_ensembl")
    gene_map_table = getBM(attributes=c('hgnc_symbol', 'entrezgene'), filters = 'hgnc_symbol', values = as.vector(unlist(gene_list)), mart = ensembl)
