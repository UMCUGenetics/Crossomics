...
# ggsave(paste0(outdir_name,"/",train_val,"_Ranks_And_Missed_",sub_name,".png"), plot = p,
#        width = 300, height = 200, dpi=resolution, units = "mm")
...
ggsave(paste0(outdir_name,"/",train_val,"_Total_genes_per_par_",sub_name,".png"), plot = p,
       width = 300, height = 200, dpi=resolution, units = "mm")
...
# ggsave(paste0(outdir_name,"/",train_val,"_Average_Patient_And_Ranks_And_Missed_",sub_name,".png"), plot = pp,
#        width = 300, height = 200, dpi=resolution, units = "mm")
ggsave(paste0(outdir_name,"/",train_val,"_test_",sub_name,".png"), plot = pp,
       width = 300, height = 200, dpi=resolution, units = "mm")
...
# ggsave(paste0(outdir_name,"/",train_val,"_Singlepanel_Av_Std_",sub_name,".png"), plot = p,
...
# tmpDT[ , P.colour := "p ≤ 0.001"]
# # tmpDT[ P.value > 0.0001, P.colour := "0.0001 < p ≤ 0.001"]
# tmpDT[ P.value > 0.001, P.colour := "0.001 < p ≤ 0.01"]
# tmpDT[ P.value > 0.01, P.colour := "0.01 < p ≤ 0.1"]
# tmpDT[ P.value > 0.1, P.colour := "0.1 < p ≤ 0.5"]
# tmpDT[ P.value > 0.5, P.colour := "0.5 < p ≤ 1"]
# tmpDT[ , P.colour := factor(P.colour, levels = c("p ≤ 0.001", "0.001 < p ≤ 0.01","0.01 < p ≤ 0.1",
#                                                     "0.1 < p ≤ 0.5", "0.5 < p ≤ 1")) ]
...
p <- ggplot(tmpDT, aes(x = PatientID, y = Position)) +
  ...
  #                    breaks = unique(DT[Z_threshold == "-3, 3" & Max_rxn == 10 & P.value != 1 & Step == 5,Patient_follow_number]),
  #                    name = "Patient ID")
ggsave(paste0(outdir_name,"/1Par_Absolute_Rank_",sub_name,".png"), plot = p,
       width = 400, height = 150, dpi=resolution, units = "mm")
...
ggsave(paste0(outdir_name,"/",train_val,"_Av_Rank_Sd_Per_Patient_",sub_name,".png"), plot = p,
       width = 300, height = 200, dpi=resolution, units = "mm")
