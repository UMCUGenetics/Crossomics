getZvalues <- function(peaklist, n_patients, n_controls, assi.lab, patients, controls, adducts=FALSE){
  ...
  if (adducts){
    ...
    z.values = matrix(NA, nrow = n_rows, ncol = 2*n_patients+n_controls)
    rownames(z.values) = peaklist[,assi.lab]
    ...
    # A check to only take rows where the HMDB code is present
    row_index=which(peaklist[,assi.lab]!="")  # ME assi.lab= HMDB_code kolom
    n_rows <- length(row_index)
    z.values = matrix(NA, nrow = n_rows, ncol = 2*n_patients+n_controls)
    rownames(z.values) = peaklist[row_index,assi.lab]
    # int.index = c((grep("theormz_noise", colnames(peaklist),fixed = TRUE)+1):(grep("avg.ctrls", colnames(peaklist))-1))
    # get only colnames starting with C or P, without _ characters after the digits.
    int.index <- grep("^(?=[CP]\\d+.\\d)(?!.*_)", colnames(peaklist), perl = TRUE)
    # int.index <- grep("[^CP]", colnames(peaklist))
  ...
  for (i in 1:n_rows){
    ...
    z.values.row = rep(NA, n_patients)
    ...
    for (j in 1:length(patients)){
      ...
      z.values.row[j] = z.average
      
      if (i==1) z.labels[j] = paste("z.average_P", patients[j], sep="")
    ...
    z.values[i,] = c(z.values.row, int.row, int.row.c) 
  }
  
  colnames(z.values) = c(z.labels, int.labels, int.labels.c)
